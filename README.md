...
- user sessions, and verifying them [./app/session.server.ts](./app/modules/auth/session.server.ts)
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create gpt-playground-2cf3
  fly apps create gpt-playground-2cf3-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app gpt-playground-2cf3
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app gpt-playground-2cf3-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'gpt-playground-2cf3-staging' does not match app name in config file 'gpt-playground-2cf3'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name gpt-playground-2cf3-db
  fly postgres attach --app gpt-playground-2cf3 gpt-playground-2cf3-db

  fly postgres create --name gpt-playground-2cf3-staging-db
  fly postgres attach --app gpt-playground-2cf3-staging gpt-playground-2cf3-staging-db
  ```
